...
export default function VoiceHandler({ isCallActive, onTranscript, onResponse }: VoiceHandlerProps) {
  ...
  useEffect(() => {
    ...
    recognition.onresult = (event: SpeechRecognitionEvent) => {
      ...
      if (lastResult.isFinal || !recognition.interimResults) {
    ...
    try {
      ...
      const response = await fetch('/api/ai-response', {
        ...
        body: JSON.stringify({
          message: userMessage,
          context: `This is during a video call meeting`,
          conversationHistory: conversationHistory.slice(-3) // Only send last 3 exchanges
        }),
  ...
  const speakResponse = useCallback((text: string) => {
    ...
    // Small delay to ensure TTS works on mobile
    setTimeout(() => {
      ...
      if (voices.length > 0) {
        // Prefer native voices on mobile
        const preferredVoice = voices.find(voice => 
          isMobile ? !voice.voiceURI.includes('Google') : voice.lang.startsWith('en')
        ) || voices[0];
